...
# Script 130 - Criar Seeder para Audiências
# Sistema Erlene Advogados - Dados de teste para audiências
# Data: $(date +%Y-%m-%d)
# EXECUTE DENTRO DA PASTA: backend/

echo "🎯 Script 130 - Criando seeder para audiências..."
...
if [ ! -f "artisan" ]; then
    ...
    echo "   cd backend"
    echo "   chmod +x 130-create-audiencias-seeder.sh && ./130-create-audiencias-seeder.sh"
    exit 1
fi

echo "1️⃣ Criando AudienciasSeeder..."

# Criar seeder
php artisan make:seeder AudienciasSeeder

# Criar conteúdo do seeder
cat > database/seeders/AudienciasSeeder.php << 'EOF'
<?php
...
echo "2️⃣ Verificando se tabela audiencias existe..."

# Verificar se a migration foi executada
php artisan migrate:status | grep "audiencias" > /dev/null
if [ $? -ne 0 ]; then
    echo "⚠️ Tabela audiências não encontrada. Executando migration..."
    php artisan migrate
fi

echo "3️⃣ Executando seeder de audiências..."

# Executar apenas o seeder de audiências
php artisan db:seed --class=AudienciasSeeder

echo "4️⃣ Verificando dados criados..."

# Verificar dados no banco
echo "📊 Estatísticas das audiências criadas:"
echo ""

# Contar total
mysql -u root -p12345678 erlene_advogados -e "SELECT COUNT(*) as 'Total de Audiências' FROM audiencias;" 2>/dev/null || echo "Erro ao conectar no banco"

echo ""
echo "📅 Audiências por status:"
mysql -u root -p12345678 erlene_advogados -e "SELECT status, COUNT(*) as quantidade FROM audiencias GROUP BY status;" 2>/dev/null || echo "Erro ao conectar no banco"

echo ""
echo "🗓️ Audiências por data:"
mysql -u root -p12345678 erlene_advogados -e "SELECT data, COUNT(*) as quantidade FROM audiencias GROUP BY data ORDER BY data;" 2>/dev/null || echo "Erro ao conectar no banco"

echo ""
echo "🏛️ Audiências por tipo:"
mysql -u root -p12345678 erlene_advogados -e "SELECT tipo, COUNT(*) as quantidade FROM audiencias GROUP BY tipo;" 2>/dev/null || echo "Erro ao conectar no banco"

echo ""
echo "✅ Seeder de audiências executado com sucesso!"
echo "✅ Dados de teste criados para desenvolvimento!"
echo ""
echo "📋 Próximo passo: Remover dados mock do AudienciaController"
echo "   chmod +x 131-remove-mock-data.sh && ./131-remove-mock-data.sh"